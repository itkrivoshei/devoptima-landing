      ...
      <div className="container grid items-center gap-12 lg:grid-cols-[1.05fr_0.95fr]">
        <div>
          ...
          <p className="mt-6 max-w-2xl text-lg leading-8 text-slate-300">
            A clean landing page concept for a DevOps services company, focused
            on automation, deployment pipelines, cloud infrastructure, and
        ...
        <div className="relative">
          <div
            className="absolute inset-0 rounded-[2rem] bg-accent/20 blur-3xl"
            aria-hidden="true"
          />
          <div className="relative rounded-[2rem] border border-white/10 bg-white/[0.03] p-5 shadow-soft">
            ...
            />
            <ol
              className="mt-4 grid gap-3 text-sm text-slate-300 sm:grid-cols-3"
              aria-label="Deployment stages"
            >
              {deploymentStages.map((stage) => (
                <li
                  key={stage}
                  className="rounded-2xl border border-accent/20 bg-background/70 px-4 py-3 text-center"
                >
                  {stage}
                </li>
              ))}
